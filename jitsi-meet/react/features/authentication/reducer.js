...
ReducerRegistry.register('features/authentication', (state = {}, action) => {
    switch (action.type) {
    // Added by vipin    
    case NSL_AUTH_FAILED:
        console.log("NSL_AUTH_FAILED reducer");
        return assign(state,{
            error: {'name':'NSL_AUTH_FAILED','message':action.payload.message},
            progress: undefined,
            thenableWithCancel: undefined
        });
    case NSL_AUTH_SUCCESS:
        return assign(state,{
            error:undefined,
            progress: {'name':'NSL_AUTH_SUCCESS','message':action.payload.message},
            thenableWithCancel: undefined
        });       
    case CANCEL_LOGIN:
        return assign(state, {
